...
from monai.losses import DiceLoss as DiceLoss_monai
...
class CustomLoss(torch.nn.Module):
    def __init__(self, loss_weight=0.5):
        ...
        self.criterion1 = DiceLoss_monai(sigmoid=True)

    def forward(self, pred_mlo, pred_cc, true_mlo, true_cc, output_mlo, output_cc):
        euclid_dist = F.pairwise_distance(output_mlo, output_cc)
        # loss = torch.mean(torch.pow(euclid_dist, 2))
        loss = torch.mean(euclid_dist)
